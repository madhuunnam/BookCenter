...
//	$query = "SELECT * FROM FROM stores,inventory,books WHERE stores.storeID=inventory.storeID and inventory.isbn=books.isbn and books.subCat like '%$subcat%'";
	//$result = mysql_query("SELECT * FROM  stores,inventory,books WHERE stores.storeID=inventory.storeID and inventory.isbn=books.isbn and storeType like '%$type%'");
	$result = mysql_query("SELECT * FROM  stores,inventory,books WHERE state like '%$type%' and stores.storeID=inventory.storeID and inventory.isbn=books.isbn group by stores.storeName");
$storelist = array();
if (!empty($result)) {
