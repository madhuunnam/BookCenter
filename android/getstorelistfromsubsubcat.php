...
//	$query = "SELECT * FROM FROM stores,inventory,books WHERE stores.storeID=inventory.storeID and inventory.isbn=books.isbn and books.subCat like '%$subcat%'";
	$result = mysql_query("SELECT * FROM  stores,inventory,books WHERE stores.storeID=inventory.storeID and inventory.isbn=books.isbn and books.subSubCat like '%$subsubcat%' group by stores.storeName");
$storelist = array();
if (!empty($result)) {
