...
<?php
 ...
 $sqlReviews = "select count(*) from StoreReviews where storeID = " .$storeId;
 $db1 = new DataBase();
	$db1->conn();
	$R= $db1->indb($sqlReviews);
	$totalReview = 0;
	while($review=mysql_fetch_row($R))
	{
		$totalReview = $review[0];
	}
	
 $sqlStars = "select avg(overallStars) from StoreReviews where storeID = " .$storeId;
 $db2 = new DataBase();
	$db2->conn();
	$S= $db2->indb($sqlStars);
	$totalStar = 0;
	while($star=mysql_fetch_row($S))
	{
		$totalStar = $star[0];
	}
?>
		<tr>
			<td bgcolor="#6699FF">
				<div style="float:left" class="coverphoto">
					<img src="storelogos/<?php echo $storeId; ?>" class="listResultImage" alt="storelogo" style="width:30px;height:20px;" />
				</div>
			</td>
			<td  bgcolor="#ddd">
				<table>
					<tr valign="top">
						<td>
							<span style="font-size:15px;"> </span><a class="menulink" href="Storepage.php?name=<?php echo $name; ?>&storeId=<?php echo $storeId; ?>" style="font-size:15px"><?php echo $name; ?></a>
						</td>
						<td class="author">
							<label style="font-size:15px"><?php echo $totalStar ; ?></label>
						</td>
						<td class="author">
							<label style="font-size:15px"><?php echo $totalReview ; ?> Reviews</label>
						</td>
					</tr>
				</table>
			</td>
