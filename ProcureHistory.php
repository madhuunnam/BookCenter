...
<body>
<h2> Procures History </h2>
<form method=post action="ProcureHistory.php">
...
<table style="text-align: left; " border="1" cellpadding="2" cellspacing="2">
  <tbody>
	...
	$getStoreNameSQL = 'select storeName from Stores where storeID=' . $_SESSION['storeID'];
	$storenameresult = mysql_query ( $getStoreNameSQL );
	while ( $row = mysql_fetch_assoc ( $storenameresult ) ) {
		$storeName = $row ['storeName'];
	}
      
      $sql = "select * from procures where StoreName = '" . $storeName . "'";
      
      if (!empty($_POST)) {
          if ($_POST['fromDate'] != '' || $_POST['toDate'] != '') {
            if ($_POST['fromDate'] != '') {
              $sql .= ' and ProcureTime >= "'.$_POST['fromDate'].'" ';
            } 
            if ($_POST['toDate'] != '') {
                $sql .= ' and ProcureTime <= "'.$_POST['toDate'].'" ';
            } 
              ...
              if ($_POST['duration'] == 'Month') {
                $sql .= ' and ProcureTime > "'.$currDate[0].'-'.$currDate[1].'-01" ';
              } else if ($_POST['duration'] == 'Year') {
                $sql .= ' and ProcureTime > "'.$currDate[0].'-01-01" ';
              } else {
                $quarter = '10';
                ...
                $sql .= ' and ProcureTime >  "'.$currDate[0].'-'.$quarter. '-01" ';
              }
          }
      }
      ...
      while ($row = mysql_fetch_assoc($result)) {
        if ($index == 0) {
          ...
          <tr>
            ...
            <th style="vertical-align: top;">Pid<br>
            </th>
            <th style="vertical-align: top;">StoreName<br>
            </th>
             </th>
            <th style="vertical-align: top;">SupplierName<br>
            </th>
            <th style="vertical-align: top;">AgentName<br>
            </th>
<!--
...
-->
            ...
            <th style="vertical-align: top;">UnitsProcured<br>
            </th>
            <th style="vertical-align: top;">TotalPrice<br>
            </th>
          </tr>
          <?php
        }
          ...
          echo "<td>" . explode(" ", $row['ProcureTime'])[0] . "</td>";
          echo "<td><a href='ProcureInfo.php?PID=" . $row['PID'] . "'>" . $row['PID'] . "</a></td>";
          echo "<td>" . $row['StoreName'] . "</td>";
          echo "<td>" . $row['SupplierName'] . "</td>";
          echo "<td>" . $row['AgentName'] . "</td>";
//          echo "<td>" . $row['totPrice'] . "</td>";
          echo "<td>" . $row['Title'] . "</td>";
          echo "<td>" . $row['UnitsProcured'] . "</td>";
