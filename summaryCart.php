...
if (isset ( $_SESSION ['custID'] )) {
	...
	if ($row ['cardNumber'] != null && $row ['cardType'] != null && $row ['cardName'] != null && $row ['cardExp'] != null && $row ['cardCode'] != null && $row ['billingAddr'] != null) {
		$hasCardInfo = true;
	}
} else {
...
if (isset ( $_SESSION ['storeID'] )) {
	...
	$disableSelfCheckout = 'disabled';  // ??? by Fu on 8-15-15 
}
...
if (! isset ( $_SESSION ['cartItems'] ) || empty ( $_SESSION ['cartItems'] )) {
	$cartIsEmpty = 'disabled';
        echo " Warning: Cannot checkout an empty cart.";
}
?>
...
<html>
<head>
<title>BookCenter: Summary Of Cart</title>
<style>
body {
	text-align: center;
}
...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$.ajax({
                ...
                success: function(response) {
 					...
 					if(responseJSON['NotAMember'] == 'true'){
 	 					alert("Please become a member of the library before getting the book!");
						window.location.href='CustomerMembership.php';	
        ...
        if (! isset ( $_SESSION ['type'] ) || $_SESSION ['type'] != 'Customer') {
                $_SESSION ['redirectUrl'] = 'summaryCart.php';
                echo "$( document ).ready(function() {" . " alert('Please Login. You will redirected back to this page after login');" . " window.location.href='Login.php';" . "});";
        }
        ?>
        function clearInput() {
...
// 										echo "<td>" . $itemJSON ['desc'] . "</td></tr>";
		...
		<table class="btnTable">
			<tr>
				<th><input type="button" name="Self-Checkout"
					<?php echo $cartIsEmpty == 'disabled' ? $cartIsEmpty : $disableSelfCheckout; ?>
					value="I'm at the library, and ready to Self-Checkout!"
					onclick="addTransaction();"></th>
			</tr>
			<tr>
				<th><input type="button" name="Cancel"
					onclick="window.location.href='homepage.php';" value="Cancel"></th>
			</tr>
			<tr>
				<th><input type="button" name="Pick-Up" <?php echo $cartIsEmpty; ?>
					onclick="pickUpItems('<?php
					...
					?>');"
					value="I'm not at the library, I'll Pick-Up later!"></th>
			</tr>
