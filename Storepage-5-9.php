...
<?php
	...
	$storeaddress = $store[4]." ".$store[6]." ".$store[7]." ".$store[8];
        $webtag = "<a ref=\"" .$store[16] ."\">" .$store[16] ."</a>"; //fu added 5/9
...
?>	
	<table class="storeInfo" align="center" style="margin:30px 10px 20px 330px">
		<tr>
			...
			<td>
				<label style="font-size:12px"><?php echo $totalreview; ?> Reviews</label>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
                ...
                <tr>
                        <td> 
                                <label style="font-size:12px">Website: <?php echo $webtag; ?></label>
                        </td>
                        <td>
...
<?php
	...
	$subCategoryCount = 0;
	$subSubCategory = null;
	$subSubCategoryCount = 0;http://localhost/bookcenter/Storepage.php?name=vid%20library&storeId=26
	$bookCount = 0;
