function uncgCart(){
	...
	me.taxRate = 0;
	me.Cost = 0;
	me.shipingFlatRate = 0;
	me.shippingTotalRate = 0;
	me.shipingQuantityRate = 0;
	me.shippingRate = 0;
	me.shippingCost = 0;
	me.currency = USD;
	me.checkoutTo = Email;
	...
	me.save = function () {
		...
		createCookie('simpledCart', dataString.substring( 2 ), 30 );
	};
	me.initializeView = function() {
		var me = this;
		me.Outlets 			= getElementsByClassName('simpleCart_total');
		me.quantityOutlets 			= getElementsByClassName('sCart_quantity');
		me.cartDivs 				= getElementsByClassName('sCart_items');
		me.taxCostOutlets			= getElementsByClassName('sCart_taxCost');
		me.taxRateOutlets			= getElementsByClassName('sCart_taxRate');
		me.shipingCostOutlets		= getElementsByClassName('sCart_shippingCost');
			...
			for( header in me.cartHeaders ){
				...
				switch( info[0].toLowerCase() ){
					case "total":
						...
						break;
					case "increment":
						outputValue = me.valueToLink( "+" , "javascript:;" , "onclick=\"sCart.items[\'" + item.id + "\'].increment();\"" );
						break;
					case "decrement":
						outputValue = me.valueToLink( "-" , "javascript:;" , "onclick=\"sCart.items[\'" + item.id + "\'].decrement();\"" );
						break;
					case "remove":
						outputValue = me.valueToLink( "Remove" , "javascript:;" , "onclick=\"sCart.items[\'" + item.id + "\'].remove();\"" );
						break;
					case "price":
				...
				for( var y=1,ylen=info.length;y<ylen;y++){
					...
					switch( option ){
						...
						case "img":
							...
							break;
						case "input":
							outputValue = me.valueToTextInput( outputValue , "onchange=\"sCart.items[\'" + item.id + "\'].set(\'" + outputValue + "\' , this.value);\""  );
							break;
						case "div":
						case "span":
						case "h1":
	...
	me.updateTotals = function() {
		...
		me.Cost = parseFloat(me.total)*me.taxRate;
...
function getCartItem() {
	...
	CartItem.protype.increment = function(){
	...
	CartItem.prototype.checkQuanlityAndPrice = function() {
...
function CartShelfItem(){
	...
	ShelfItem.prototype.addToCart = function () {
		...
		for( var field in this ){
			if( typeof( this[fields] ) != "function" && field != "id" ){
				valueString = "";
...
function eraseCartCookie(name) {
	createCookie(name,"",-1);
}

var getCartElementsByClassName = function (className, tag, elm){
