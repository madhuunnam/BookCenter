	...
	function holdThisBook(isbn, holderId, holdDate, holdDays, storeId, storeName) {
		...
		if (holderId.length <= 0 || holdDate.length <= 0){
        	...
        	var newHoldDateString = newHoldDate.toString('yyyy-MM-dd');
                ...
                method: "POST",
                url: "holdBook.php",
                data:    "&holderId="+ currentCustId +"&isbn="+isbn 
...
?>	
	<table class="storeInfo" align="center"
				...
				<tr>
					...
					<td><label style="font-size: 12px">Self Checkout: <?php echo $selfCheckOut; ?> </label>
					</td>
				</tr>
										...
										// if($cateogryID != -1)
										{
											...
											while ( $subcattabs = mysql_fetch_row ( $subCatsSQLResults ) ) {
												...
												$getbooksForSubCat = "select B.title,B.author,I.quantity,B.isbn,I.privateCallNum,B.translator,B.editionType,
		                										...
		                										B.language,B.audience,B.productFormatDetail,B.fromBackCover,B.contents,B.subSubCat,B.keywords,B.productDimensions,B.shippingWeight,I.holdDate,S.dueHold
												...
												while ( $bookRow = mysql_fetch_row ( $booksForSubCatSQLResults ) ) {
													...
													if ($inventoryQuantity == 0 || ($inventoryQuantity == 1 & $holderID != '')) {
														...
														echo "<td><input type='button' style='color:#fff;width:100%;background-color:#585858;border-color:#C8C8C8' name='holdit' onclick='holdThisBook(\"$isbn\",\"$holderID\",\"$holdDate\",\"$dueHold\",\"$storeid\",\"$storeName\");' value='Hold It' /></td></tr>";
								...
								<td colspan=3 style="text-align: left;"><input type="text"
									id='<?php echo $subLinkID; ?>Desc' size=15 value=''></td>
											...
											if (mysql_num_rows ( $k ) == 0) {
												echo "<p>No books in this category yet</p>";
											}
										}
										?>
										</div>
				...
				<div class="block1" id="rightNav">
					<table id="Category" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<a href='coverimages/didyouknow.jpg' data-lightbox='coverimages/didyouknow.jpg'><img src ='coverimages/didyouknow.jpg' 
							class='listResultImage'  alt='didyouknowimg' style='width:200px;height:120px;' /></a>
							</td>
						</tr>
						<tr>
							<td style="width: 200px; height: 45px; border: none; text-align:left;"  />
							<a href="http://www.ultracalc.com"><i><strong>UltraCalculators are fun
							 for daily use and kid's homework checking</strong></i></a>
							</td>
						</tr>
						<tr>
							<td style="width: 200px; height: 45px; border: none;" />
